...
import { useUpdateRoomAdminMutation } from "../../store/services/hotelRooms";
...
export default function UpdateRoomPage() {
  ...
  const [updateRoomAdmin, {isLoading, isError}] = useUpdateRoomAdminMutation();
  ...
  const updateRoom = async (e: React.FormEvent) => {
    try {
      ...
      const resultHotelRoom = await updateRoomAdmin({_id: id, data: formData}).unwrap();
